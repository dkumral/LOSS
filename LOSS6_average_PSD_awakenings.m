...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
load('/home/kumral/Desktop/Projects/LOSS_analyses/github_scripts/combined_data_interpolate.mat');
...
for stg = 1:5
    combined_data.Pows(ind,1).res_red{1, stg}   =NaN(size(combined_data.Pows(ind,1).res_red{1, stg}));  %LOSS_VP15_3
    combined_data.PSD_avg_epoch_rej{ind,stg}  = NaN(size(combined_data.PSD_avg_epoch_rej{ind,stg})); %9_4
end
...
for stg = 1:5
    for subj = 1:20
        ...
        for ch = 1:32
            for i = 1:length(ind)
                PSD_xx = combined_data.PSD_avg_epoch_rej{ind(i),stg}(ch,:); %arrange for the original PSD
                ...
                PSD_x = combined_data.Pows(ind(i),1).res_red{1, stg}(ch,:);
        ...
        data_reduced(subj).PSD_org_red{1, stg} = reshape(PSD_mean_org.',1,[])';
...
save data_reduced_interpolate data_reduced
%%