...
for subj=1:81
    ...
    dir = strcat(data(subj).filename(1:10),'_real_rejected_epochs_interpolated');
    for stg = 1:5
        nexttile()
        for n = 1:32
...
for subj=1:81
    ...
    x_width=28 ;y_width=7;
    set(gcf, 'PaperPosition', [0 0 x_width y_width]); %
    dir = strcat(data(subj).filename(1:10),'_real_notrejected_epochs_interpolated');
    for stg = 1:5
        nexttile()
        for n = 1:32
            command = [ 'disp(''x ' num2str(n) ''')' ];
...
for subj=1:81
    ...
    x_width=28 ;y_width=7;
    set(gcf, 'PaperPosition', [0 0 x_width y_width]); %
    dir = strcat(data(subj).filename(1:10),'_res_corrected');
    for stg = 1:5
        nexttile()
        for n = 1:32
            command = [ 'disp(''x ' num2str(n) ''')' ];
